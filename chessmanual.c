...
static void countKeys(Key line, int count)
{
    if (count == 5) {
...
static int countPower(Line line, PatternLib pattern)
{
    int power = 0, mask, p;
...
static void countLib(ChessManual manual, Line line, int count, PatternLib pattern)
{
    if (count == LLN) {
